...
def cross_entropy_domain_adaptation(logits_dict, y, batch=None):
    logits = logits_dict['logits']
    logits = logits.view(-1, logits.shape[-1])
    y = y.view(-1)
    if batch is not None:
        batch_logits = logits_dict['batch_logits']
        batch = batch.view(-1)
        return F.cross_entropy(logits, y) + F.cross_entropy(batch_logits, batch)
    return F.cross_entropy(logits, y)


def weighted_cross_entropy(logits, y, weight, ignore_index=-100):
    logits = logits.view(-1, logits.shape[-1])
    y = y.view(-1)
    return F.cross_entropy(logits, y, weight=weight[0], ignore_index=ignore_index)
...
def mse(outs, y, ignore_index=-100):
    loss = (outs - y) ** 2
    return loss[y != ignore_index].mean()
...
output_metric_fns = {
    ...
    "cross_entropy_domain_adaptation": cross_entropy_domain_adaptation, 
